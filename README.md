...
在基于生成式视频模型（如 Google Flow / Omni 1.1 Flash）进行多镜头短剧或故事创作时，核心痛点是**多镜头间的画风与角色一致性**。`styleforge` 提供两种并存的生产模式，不预设结论，由创作者根据实际题材与数据决定：
...
| **自测工具** | `styleforge smoke`（单镜端到端验证） | `styleforge drift-test`（逐跳 MAD 漂移量化分析） |

> 💡 **项目取证与双闸机制：**  
> 在过往自动化实践中，我们曾发现网络异常导致的 14 字节假视频（如 `No session found` 响应页）被误存为 `.mp4`，导致接力链条物理断裂与误判。因此 `styleforge` 内置了：
...
校验剧本语法与参数合法性：
```bash
styleforge check my-story.yaml
```

### 3. 离线演练（零点数消耗）
使用内存假驱动（`--fake`）模拟全流程，验证文件流转与断点机制：
```bash
styleforge run my-story.yaml --fake
```

### 4. 冒烟测试与接力漂移实验
```bash
# 离线模拟单镜冒烟
styleforge smoke --fake

# 真实真机冒烟（需显式 --yes 授权，消耗真实 Flow 积分）
styleforge smoke --yes

# 尾帧接力漂移实验（2-3 跳量化评测，输出单跳与累积 MAD 报告）
styleforge drift-test --shots 3 --fake
```
...
1. **非官方工具声明**：`styleforge` 为开源的个人研究与效率辅助工具，与 Google 或 Google Flow 官方无任何隶属或背书关系。
