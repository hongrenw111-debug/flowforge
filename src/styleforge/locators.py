...
LOCATORS: dict[str, Locator] = {
    ...
    "start_slot": Locator(
        key="start_slot",
        description="底栏 Start 首帧槽（aria-haspopup=dialog 的 Radix 触发器）",
        role="button",
        name="Start",
        match="exact",
    ...
    "end_slot": Locator(
        key="end_slot",
        description="底栏 End 尾帧槽（aria-haspopup=dialog 的 Radix 触发器；v1 不用，留作尾帧扩展）",
        role="button",
        name="End",
        match="exact",
        css='[aria-haspopup="dialog"]',
